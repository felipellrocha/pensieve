...
class Kubernetes {
  ...
  getService = async name => {
    ...
    for (let service of res.body.items) {
      if (service && service.metadata && service.metadata.name === name) return service;
    }
  ...
  waitForPodInit = name => {
    ...
    return new Promise(function(resolve, reject) {
      const t = watcher.watch(`/api/v1/namespaces/${namespace}/pods`, {}, (phase, obj) => {
        if (
          phase === 'MODIFIED' &&
          (obj && obj.metadata && obj.metadata.labels && obj.metadata.labels.name === name) &&
